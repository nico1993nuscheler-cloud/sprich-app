...
final class NetworkStatusIndicator: ObservableObject {
    ...
    enum Route: Equatable {
        ...
        /// User-facing short label. Note the `.offline` case reads
        /// **"On this Mac"**, not "Offline": v1.0.6 QA confirmed users
        /// parse "Offline" as "Sprich itself isn't working" rather than
        /// "this dictation never leaves my Mac." The internal enum case
        /// stays `.offline` for code clarity, but every user-visible
        /// surface (menubar, Settings → Privacy, tooltip) speaks the
        /// same provider-card language ("On this Mac").
