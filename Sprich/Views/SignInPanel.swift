...
/// The shared sign-in surface. Used both by the standalone window
/// (`SignInView`) and inline by step 1 of `OnboardingView`.
///
/// Drives `AuthService.shared` directly. Exposes a `showsHeader` flag
/// so the standalone window renders its own welcome header while the
/// onboarding step suppresses it (the onboarding card already has one).
///
/// Three OAuth providers ship in the panel — Apple / Google / Microsoft —
/// but Microsoft is hidden for Phase 1 (Sprint 2B decision: Microsoft
/// requires Partner-publisher verification for personal-account consent;
/// Phase 2 M365 unlocks this cleanly).
