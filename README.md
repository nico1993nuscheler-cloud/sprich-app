...
`Sprich.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved` is
checked into the repo on purpose — it pins every Swift Package dependency to
the exact revision that ships in the signed binary. Don't regenerate it as a
drive-by; bumps go through review.
...
- **Clipboard safety**: Sprich saves and restores your existing clipboard, and
  marks its paste with the `org.nspasteboard.ConcealedType` /
  `AutoGeneratedType` hints respected by Maccy, Paste, Raycast Clipboard,
  Alfred, and Pastebot — so dictations don't pollute your clipboard history.
  Apps that don't follow the [nspasteboard.org](https://nspasteboard.org)
  convention will still capture the entry; that's a limitation of any
  paste-based dictation tool on macOS.
